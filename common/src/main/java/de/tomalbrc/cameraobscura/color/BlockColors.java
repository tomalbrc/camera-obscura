...
public class BlockColors {
    ...
    private static void loadColorMaps() {
        try {
            GRASS_TEXTURE = ImageIO.read(new ByteArrayInputStream(RPHelper.loadTextureBytes(CachedIdentifierDeserializer.get("colormap/grass"))));
            FOLIAGE_TEXTURE = ImageIO.read(new ByteArrayInputStream(RPHelper.loadTextureBytes(CachedIdentifierDeserializer.get("colormap/foliage"))));
            DRY_FOLIAGE_TEXTURE = ImageIO.read(new ByteArrayInputStream(RPHelper.loadTextureBytes(CachedIdentifierDeserializer.get("colormap/dry_foliage"))));
    ...
    public static void init() {
        ...
        colors.put(Blocks.DEAD_BUSH, dryFoliageColor);
    ...
    public static int get(ChunkAccess level, BlockState blockState, BlockPos blockPos) {
    ...
    public static int biomeWaterColor(ChunkAccess level, BlockPos blockPos) {
        return 0xFF << 24 | getBiome(level, blockPos).value().getWaterColor();
    }

    private static Holder<Biome> getBiome(ChunkAccess chunk, BlockPos blockPos) {
    ...
    interface BlockColorProvider {
        int get(ChunkAccess level, BlockState blockState, BlockPos blockPos);
    }
}
